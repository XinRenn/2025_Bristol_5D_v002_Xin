...
# # 初始绘图
# contour = ax.contourf(X, Y, predic_var['mean'][0], cmap='coolwarm', levels=np.linspace(-max(abs(np.min(predic_var['mean'])), abs(np.max(predic_var['mean']))), max(abs(np.min(predic_var['mean'])), abs(np.max(predic_var['mean']))), 41))
